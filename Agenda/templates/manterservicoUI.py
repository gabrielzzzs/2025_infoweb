    ...
    def atualizar():
        ...
        if len(servicos) == 0:
            ...
            op = st.selectbox(
                ...
                servicos,
                format_func=lambda s: f"{s.nome} (R$ {s.preco:.2f})"
            )
            nome = st.text_input("Novo nome", op.nome)
            preco = st.number_input("Novo preço", value=op.preco, step=0.01, format="%.2f")
            if st.button("Atualizar"):
                View.servico_atualizar(op.codigo, nome, preco)
    ...
    def excluir():
        ...
        if len(servicos) == 0:
            ...
            if st.button("Excluir"):
                View.servico_excluir(op.codigo)
